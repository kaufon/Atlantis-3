...
export class CreateClientUseCase {
  ...
  public async execute(): Promise<void> {
    ...
    const registrationDate = await this.getValidDate(
      "Insira a data do registro (DD/MM/YYYY):",
    );
  ...
  private async getCellphones(): Promise<Cellphone[]> {
    const quantity = await this.input.numberInput(
      "Quantos telefones deseja adicionar?",
    );
  ...
  private async getDocuments(): Promise<Document[]> {
    const quantity = await this.input.numberInput(
      "Quantos documentos deseja adicionar?",
    );
    const documents: Document[] = [];

    for (let i = 0; i < quantity; i++) {
      const type = (await this.input.selectInput(
      ...
      const number = await this.input.textInput(
        "Insira o número do documento:",
      );
      const expeditionDate = await this.getValidDate(
        "Insira a data de emissão do documento (DD/MM/YYYY):",
      );

      documents.push(
        new Document({
          number,
          expeditionDate: parseDate(expeditionDate),
          type,
        }),
      );
    }
